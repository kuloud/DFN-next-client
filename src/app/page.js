...
export default function Home() {
  ...
  const [urlInput, setUrlInput] = useState(
    "https://greenfield-sp.defibit.io/gata-task-01/0x01/963471b1401d529046b655b77aa0b6c2"
  );
    ...
    const onMessageReceived = (e) => {
      ...
      switch (e.data.status) {
          ...
          if (progressData.total > 0) {
            ...
            if (progressData.progress === 100) {
              setCompletedDownloads((prev) =>
                new Set(prev).add(progressData.file)
              );
            }
          }
          break;
        case "ready":
          ...
          break;
        case "log":
          const [label, ...args] = e.data.args;
          const logger = e.data.level === "error" ? console.error : console.log;
          logger(`[Worker] ${label}`, ...args);
          break;
        case "complete":
          console.log("Message received:", e.data);
  ...
  const isSubmitDisabled = isLoading || !(textInput.trim() && urlInput.trim()); // Require both inputs
        ...
        <Card className="w-full max-w-3xl p-6 flex flex-col">
          ...
          <div className="flex justify-center mb-6">
            <Button
              ...
              onPress={() => submit(textInput, urlInput)}
          ...
          <div className="space-y-4">
            {/* Progress Display */}
            {downloads.size > 0 &&
              Array.from(downloads.values()).some((d) => d.total > 0) && (

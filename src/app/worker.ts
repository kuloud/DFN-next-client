...
class DFN {
  ...
  async initialize(
    modelName: string = "XudongShen/DFN-public",
...
self.addEventListener("message", async (event) => {
  ...
  const dfn = await PipelineSingleton.getInstance((x) =>
  ...
  function padToSquare(img: RawImage): RawImage {
    ...
    for (let y = 0; y < height; y++) {
      for (let x = 0; x < width; x++) {
        ...
        const destIndex =
          ((y + offsetY) * maxDim + (x + offsetX)) * img.channels;
  ...
  const computeSimilarity = async (text: string, imageUrl: string) => {
    ...
    try {
      ...
      for (const chunk of tokenChunks) {
        ...
        const chunkEmbedding = normalize(
          textOutputs.text_embeds.ort_tensor.cpuData
        );
